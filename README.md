...
| FAKE_DATA_SCOPE            | Set the scope where data generated by FakeIt will be stored                      |
| FAKE_DATA_COLLECTION_FIELD | Set the model field to use as the collection name where fake data will be stored |
...
**IMPORTANT:** If you plan to generate fake data and want your data to be segmented by model type, you need to make sure the collection names you provide matches the field in the model you are going to use as the collection name. For example, in the `example/sample/models/users.yaml` document there is a property called `type`. The value of that property is `user`. We need to specify the value of `user` as one of the `collections` array items and provide an ENV key/value of `FAKE_DATA_COLLECTION_FIELD="type"`. The use of the ENV key/value is defined in more detail below under [FakeIt usage with Couchbase 7](#fakeit-usage-with-couchbase-7).
...
When using Couchbase 7 or greater, data generated by FakeIt will be inserted into the `_default` scope and `_default` collection. If you would like to insert the data into a different scope and collection you must have created that scope and collection in Couchbase. See the [Scopes and Collections](#scopes-and-collections) section above on automating the creation of scopes and collections. Additionally, you must specify the following `ENV` variables in your Dockerfile or Docker Compose file:

```text
FAKE_DATA_SCOPE
FAKE_DATA_COLLECTION_FIELD
```

For example, in Docker Compose you would add something like below:
...
services:
  couchbase:
    environment:
      FAKE_DATA_SCOPE="my_scope"
      FAKE_DATA_COLLECTION_FIELD="type"
```

You can omit either environment variable. If you do, the value of `_default` will be used. The `FAKE_DATA_COLLECTION_FIELD` should be the name of your model field that you would like to use for the collection name. **IMPORTANT:** This name must match the name of an existing collection. Again, the collection creation can be automated by following the steps in [Scopes and Collections](#scopes-and-collections)

The one downside as of 2020-12-18 is that you are limited to generating data to a single scope. You can use multiple collections though.
